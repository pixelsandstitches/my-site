...
// ----------------------------------------------------------------------------
// Beachcrest — Google Sites constrained mockup
// Honest simulation of what Google Sites (Impression theme, teal accent,
// Spectral heading font) can actually produce. Constraints honored:
//   - No custom card shadows or colored top-borders
//   - No tight spacing control (sections have fixed padding)
//   - No custom button colors beyond the theme accent
//   - No hero gradient overlay (Google Sites overlays text on a plain tint)
//   - Alternating white / sand section backgrounds only
//   - Collapsible text is Google's native block (plain expand/collapse)
//   - Typography: Spectral (serif) headings + system sans body
//   - One theme color (teal) applied automatically to buttons, links, nav
//   - Nav is auto-generated by Google from page names — simple top bar
// ----------------------------------------------------------------------------
...
// Google Sites nav — flat, auto-generated, no active state styling beyond underline
  ...
  return (
    <header style={{ background: TEAL, color: "#fff" }}>
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "0 20px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64 }}>
        ...
        {/* Google Sites nav: just text links, underline on active, no pill/card styling */}
...
// Google Sites banner — image with a plain semi-transparent teal tint overlay
// No gradient control, text auto-centered by the tool
function GSBanner({ title, sub, short }) {
  return (
    <div style={{ position: "relative", height: short ? 200 : 420, overflow: "hidden" }}>
      ...
      {/* Google Sites applies a flat tint, not a gradient */}
      <div style={{ position: "absolute", inset: 0, background: "rgba(22,56,63,0.55)" }} />
      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", textAlign: "center", padding: "0 24px", color: "#fff" }}>
        <h1 style={{ fontFamily: "'Spectral', Georgia, serif", fontSize: short ? 32 : 52, margin: 0, fontWeight: 600, lineHeight: 1.1 }}>{title}</h1>
        {sub && <p style={{ fontSize: 18, marginTop: 12, maxWidth: 560, lineHeight: 1.5, opacity: 0.92 }}>{sub}</p>}
      </div>
    </div>
  );
}

// Google Sites section — fixed padding, two background options only
...
// Google Sites heading — Spectral serif, no custom color beyond default dark
...
// Google Sites four-column layout
function GSFour({ items }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 20 }} className="gs-four">
      {items.map((it, i) => (
        <div key={i} style={{ padding: "16px 0", borderTop: `2px solid ${TEAL}` }}>
          <div style={{ fontFamily: "'Spectral', serif", fontSize: 16, fontWeight: 600, color: INK, marginBottom: 6 }}>{it.title}</div>
...
// Google Sites button — teal fill, no customisation beyond theme color
...
// Google Sites collapsible text — plain native expand/collapse, no styling
    ...
    <div style={{ border: `1px solid ${LINE}`, borderRadius: 4, overflow: "hidden" }}>
      {items.map((it, i) => (
        <div key={i} style={{ borderTop: i ? `1px solid ${LINE}` : "none" }}>
          ...
          {open === i && (
            <div style={{ padding: "12px 16px 16px", background: WHITE, fontSize: 15, lineHeight: 1.65, color: INK, borderTop: `1px solid ${LINE}` }}>
              {it.a}
            </div>
          )}
        </div>
...
// Google Sites link list — plain underlined links, no styling beyond color
...
// Google Sites table — basic bordered table only
...
function Home({ go }) {
  return <>
    ...
    <GSSection bg={SAND}>
      ...
      <GSFour items={[
        { title: "About", sub: "Board, meetings & how we run" },
        { title: "Documents & Rules", sub: "Bylaws, covenants, policies, forms" },
        { title: "Resident Resources", sub: "Marina, FAQs & links" },
        { title: "News & Events", sub: "Calendar & The VIEW newsletter" },
      ]} />
    </GSSection>
    <GSSection>
      <GSTwo
    ...
    <GSSection>
      <GSTwo
        ...
        right={<>
          ...
          <ul style={{ paddingLeft: 18, margin: 0, fontSize: 15, lineHeight: 1.8, color: INK }}>
...
function GSFooter({ go }) {
  return (
    <footer style={{ background: "#2c2c2c", color: "#ccc", padding: "32px 20px" }}>
      ...
      {/* Google Sites always shows this footer attribution */}
...
function ConstraintBanner() {
  return (
    <div style={{ background: "#2d2d2d", color: "#f0c070", padding: "10px 20px", fontSize: 13, textAlign: "center", fontFamily: "system-ui, sans-serif" }}>
      <strong>Google Sites simulation</strong> — honest representation of tool constraints: Spectral/system-sans fonts, flat tint hero (no gradient control), plain section backgrounds, no custom card styling, native collapsible FAQs, single theme color.
    </div>
  );
}
